...
const listMovies = async (options = {}) => {
  ...
  if (q) {
    const like = `%${q}%`;
    filters.push("(m.title LIKE ? OR m.original_title LIKE ? OR m.slug LIKE ?)");
  ...
  const sortMap = {
    ...
    rating: "COALESCE(rating_stats.avg_rating, 0) DESC, COALESCE(rating_stats.rating_count, 0) DESC",
